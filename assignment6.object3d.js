...
class ShadedObject3D extends Object3D { 
    ...
    createVAO( gl, shader )
    {
        ...
        // `stride` changes based on whether model has a texture or not
        stride = Float32Array.BYTES_PER_ELEMENT * num_total_components  // 24 or 44

        let location = shader.getAttributeLocation( 'a_position' )
        if (location >= 0) {
            // TODO: Set up position attribute
            offset = 0
            shader.gl.vertexAttribPointer(location, this.num_components_vec3, gl.FLOAT, false, stride, offset)
        ...
        if (location >= 0) {
            ...
            offset = Float32Array.BYTES_PER_ELEMENT * this.num_components_vec3  // 12
            shader.gl.vertexAttribPointer(location, this.num_components_vec3, gl.FLOAT, false, stride, offset)
            shader.gl.enableVertexAttribArray(location)
        }
        ...
        if (location >= 0 && this.material.hasTexture()) {
            ...
            offset = Float32Array.BYTES_PER_ELEMENT * this.num_components_vec3 * 2  // 24
            shader.gl.vertexAttribPointer(location, this.num_components_vec3, gl.FLOAT, false, stride, offset)
            shader.gl.enableVertexAttribArray(location)
        }
        ...
        if (location >= 0 && this.material.hasTexture()) {
            ...
            offset = Float32Array.BYTES_PER_ELEMENT * this.num_components_vec3 * 3  // 36
            shader.gl.vertexAttribPointer(location, this.num_components_vec2, gl.FLOAT, false, stride, offset)
            shader.gl.enableVertexAttribArray(location)
        }
